...
homes_sub <- filter(homes, City %in% c("San Luis Obispo", "Santa Maria-Orcutt", "Atascadero", "Arroyo Grande"))
...
# a. Saturated model: 
homes_lml <- lm(Price ~ City + Bedrooms + Bathrooms + SqFt + PricePerSqFt + Status, data = homes_sub)
...
homes_lm2 <- lm(Price ~ SqFt + Status + City, data = homes_sub)
...
homes_sub$Status <- as.factor(homes_sub$Status)

# b. Check to ensure it's a factor now
is.factor(homes_sub$Status)
...
homes_lm3 <- lm(Price ~ SqFt + Status + City, data = homes_sub)
summary(homes_lm3)
...
sat_aic <- AIC(homes_lml)
sat_aic
final_aic <- AIC(homes_lm3)
final_aic
# Answer: which would you pick? 
# With AIC you pick lml, but conceptually should pick lm3, because: overall, heteroscedasticity is ok, residuals normality definitely looks good, and conceptually and mathematically my model is making sense!
```
...
lm_table <- stargazer(homes_lml, homes_lm3, type = "html")
...
price_predict <- as.data.frame(predict(homes_lm3, newdata = df_new, se.fit = TRUE, interval = "confidence"))
